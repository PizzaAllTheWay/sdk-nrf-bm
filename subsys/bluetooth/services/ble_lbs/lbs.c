...
uint32_t ble_lbs_on_button_change(struct ble_lbs *lbs, uint16_t conn_handle, uint8_t button_state)
{
	...
	nrf_err = sd_ble_gatts_value_get(conn_handle, lbs->button_char_handles.cccd_handle, &cccd_val);
	...
	/*
	 * Send notification using the current GATT database value (p_data = NULL).
	 * The database was already updated above, so no second write occurs here.
	 */
