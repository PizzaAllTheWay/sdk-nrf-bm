...
static struct ble_adv_data adv_data = {
...
static bool device_name_load(uint8_t *buf, uint16_t *len)
{
...
int main(void)
{
	...
	bool name_found;
	uint8_t device_name[BLE_GAP_DEVNAME_MAX_LEN];
	 ...
	 */
	device_name_len = sizeof(device_name);
	name_found = device_name_load(device_name, &device_name_len);
	if (name_found) {
		LOG_INF("Loaded saved device name");
		LOG_HEXDUMP_INF(device_name, device_name_len, "Name");
	} else {
		LOG_INF("No saved name, using default");
		device_name_len = strlen(CONFIG_SAMPLE_BLE_DEVICE_NAME);
		memcpy(device_name, CONFIG_SAMPLE_BLE_DEVICE_NAME, device_name_len);
	}

	BLE_GAP_CONN_SEC_MODE_SET_OPEN(&device_name_sec);
	nrf_err = sd_ble_gap_device_name_set(&device_name_sec, device_name, device_name_len);
	if (nrf_err) {
		LOG_ERR("Failed to set device name, nrf_error %#x", nrf_err);
		goto idle;
	}

	LOG_INF("Device name set to: %.*s", device_name_len, device_name);
