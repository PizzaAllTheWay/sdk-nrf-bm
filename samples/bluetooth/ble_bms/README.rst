...
.. note::
   The default authorization code `ABCD` (ASCII `41 42 43 44`) is configurable through Kconfig by changing the `BLE_BMS_AUTHORIZATION_CODE` option in the samples Kconfig file. The maximum allowed length is `BLE_BMS_AUTH_CODE_MAX_LEN` (127 bytes), which comes from the BMS control/point definition.
