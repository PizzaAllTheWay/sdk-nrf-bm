 ...
 *
 * @details Reads the input report CCCD via @ref sd_ble_gatts_value_get for @p conn_handle.
 *          If notifications are enabled, stores the report in the per-connection context
 *          and sends it with @ref sd_ble_gatts_hvx. If notifications are not enabled, or
 *          the CCCD cannot be read, the function fails and does not update the host context
 ...
 *
 * @details Reads the boot keyboard input report CCCD via @ref sd_ble_gatts_value_get.
 *          If notifications are enabled, stores the report in the per-connection context
 *          and sends it with @ref sd_ble_gatts_hvx. Otherwise the function fails and does
 *          not update the host context or the characteristic value.
 *
 * @param[in] hids HID Service structure.
 * @param[in] conn_handle Connection handle.
 ...
 *
 * @details Reads the boot mouse input report CCCD via @ref sd_ble_gatts_value_get.
 *          If notifications are enabled, stores the report in the per-connection context
 *          and sends it with @ref sd_ble_gatts_hvx. Otherwise the function fails and does
 *          not update the host context or the characteristic value.
 *
